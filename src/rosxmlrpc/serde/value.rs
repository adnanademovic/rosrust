...
impl XmlRpcRequest {
    pub fn new<T: std::io::Read>(body: T) -> Result<XmlRpcRequest> {
        let tree = Tree::new(body).chain_err(
            || "Failed to transform XML data into tree",
        )?;
        ...
        match method.peel_layer("methodName").chain_err(
            || "Bad XML-RPC Request method name value",
        )? {
...
impl XmlRpcValue {
    pub fn new<T: std::io::Read>(body: T) -> Result<XmlRpcValue> {
        XmlRpcValue::from_tree(Tree::new(body).chain_err(
            || "Couldn't generate XML tree to form value",
        )?)
    }
    ...
    fn read_member(tree: Tree) -> Result<(String, XmlRpcValue)> {
        let (key, mut children) = match tree {
            ...
            Tree::Leaf(_) => {
                bail!(
                    "Structure member node should contain a node called 'member' with two \
                       children"
                )
            }
        };
        ...
        let name = match name_node.peel_layer("name").chain_err(
            || "First struct member field should be a node called 'name'",
        )? {
            Tree::Leaf(name) => name,
            Tree::Node(_, _) => {
        ...
        XmlRpcValue::from_tree(value)
            .chain_err(|| {
                format!("Failed to parse subtree of struct field {}", name)
            })
            .map(|v| (name, v))
    }

    fn from_parameter(tree: Tree) -> Result<XmlRpcValue> {
        XmlRpcValue::from_tree(tree.peel_layer("param").chain_err(
            || "Parameters should be contained within node 'param'",
    ...
    fn from_tree(tree: Tree) -> Result<XmlRpcValue> {
        ...
        if key == "struct" {
            return Ok(XmlRpcValue::Struct(values
                .into_iter()
        ...
        if key == "array" {
            return if let Some(Tree::Node(key, children)) = values.pop() {
                if key != "data" {
                    bail!(
                        "Node 'array' must contain 'data' node, but '{}' detected",
                        key
                    );
                }
        ...
        match key.as_str() {
            "i4" | "int" => {
                Ok(XmlRpcValue::Int(value.parse().chain_err(|| {
                    format!("Failed to parse integer (i32) {}", value)
                })?))
            }
            "boolean" => {
                Ok(XmlRpcValue::Bool(
                    value.parse::<i32>().chain_err(|| {
            ...
            "double" => {
                Ok(XmlRpcValue::Double(value.parse().chain_err(|| {
                    format!("Failed to parse double (f64) {}", value)
                })?))
            }
...
impl Tree {
    fn new<T: std::io::Read>(body: T) -> Result<Tree> {
        parse_tree(&mut xml::EventReader::new(body))?.ok_or(
            "XML data started with a closing tag"
                .into(),
        )
    }
...
fn parse_tree<T: std::io::Read>(reader: &mut xml::EventReader<T>) -> Result<Option<Tree>> {
    match next_node(reader).chain_err(|| "Unexpected end of XML data")? {
        ...
        Node::Open(name) => {
            ...
            while let Some(node) = parse_tree(reader).chain_err(
                || ErrorKind::TreeParsing(name.clone()),
            )?
            {
                children.push(node);
            }
...
<methodCall>
  ...
  <params>
    ...
    fn writes_string() {
        assert_eq!(
            r#"<value><string>First test</string></value>"#,
    ...
    fn writes_float() {
        assert_eq!(
            r#"<value><double>33.25</double></value>"#,
